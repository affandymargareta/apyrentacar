              ...
              <div class="projects-inner">
                  ...
                  <!-- Modal -->
                    <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                      ...
                      <form method="post" action="{{ route('cityprice.store') }}" enctype="multipart/form-data">
                        @method('POST')
                        @csrf
                              ...
                              <div class="form-group" style="color: #000;">
                                <label >Province</label>
                                  <select name="province_id" style="background: #f8f9fa !important; color: #0044cc !important;" class="rounded-pill form-control">
                                      ...
                                      <option value="{{ $row->id }}" {{ old('province_id') == $row->id ? 'selected':'' }}>{{ $row->province }}</option>
                                      @endforeach
                                  </select>  
                                  <small class="form-text text-muted">We'll never share your Province with anyone else.</small>
                              </div>

                              <div class="form-group" style="color: #000;">
                                <label>Zona</label>
                                <select name="zona" style="background: #f8f9fa !important; color: #0044cc !important;" class="rounded-pill form-control">
                                  <option value="1">Zona 1</option>
                                  <option value="2">Zona 2</option>
                                  <option value="3">Dalam Kota</option>
                                </select>
                              </div>
                  ...
                  <div class="container" style="color: #fff;">
                      <table id="example" class="table table-striped table-bordered" style="color: #fff;">
                          ...
                          <tbody>
                          @foreach ($city as $getData)
                            <tr>
                            <td>{{ $loop->iteration }}</td>

                              <td>
                              {{ $getData ->product_id }}  <br> {{ $getData ->productName->name }} 
                              </td>
                              <td>
                                  {{ $getData ->province_id }}  <br> {{ $getData->province->province }}
                              ...
                              <td>
                                <form action="{{ route('cityprice.destroy', $getData->id) }}" method="post">
                                  @csrf
                                  @method('DELETE')
                                  <a href="{{ route('cityprice.edit', $getData->id) }}" type="button" class="btn btn-info">Edit Product</a>
