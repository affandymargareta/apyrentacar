		...
		<div class="container">
			...
			<div class="row">
					...
					<div class="card">
						...
						<div class="card-body">
							<ul class="list-group list-group-flush">
								<li class="list-group-item"><h6 class="card-title" style="color : #000;">{{ $cart->dengansopir->productName->name }}</h6></li>
				...
				<div class="col-lg-8 order-md-1">
					...
					<form method="post" action="{{ route('cartupdate2', $cart->id) }}" enctype="multipart/form-data">               
                      @method('PUT')
                      @csrf
						<div class="mb-3">
					...
					<ul class="list-group" style="margin-top: 15px;">
						@if ($cart->lokasi_jemput > 0)
							@if ($cart->jemput_id != $cart->dengansopir->wilayah)
					...
					<ul class="list-group" style="margin-top: 15px;">
						@if ($cart->lokasi_kembali > 0)
							@if ($cart->kembali_id != $cart->dengansopir->wilayah)
						...
						<div class="panel-group" id="accordion">
							<div class="panel panel-default">
								...
								<div class="card-body">
									...
									<form method="post" action="{{ route('orders2.store') }}" enctype="multipart/form-data">
									@method('POST')
									@csrf
									...
									<div class="panel-body">
										<ul class="list-group">
											<li class="list-group-item d-flex justify-content-between align-items-center" style="color : #000;">
												{{ $cart->dengansopir->productName->name }} x {{ $cart->durasi }}
												<span style="color : #000;">{{ formatUang($cart->dengansopir->price * $cart->durasi)}}</span>
											</li>
											@if ($cart->jemput_id > 0)
												@if ($cart->jemput_id != $cart->dengansopir->wilayah)
													<li class="list-group-item d-flex justify-content-between align-items-center" style="color : #000;">
